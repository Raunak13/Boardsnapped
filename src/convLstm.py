...
def format_data(in_x,in_y,timestep):
	...
	y_train=np.zeros((total_samples-timestep,timestep));
	
	offset=int(timestep/2);
	print('offset= '+str(offset));
	for i in range(dim[0]):
		x_train[i]=in_x[i:i+timestep];
		y_train[i]=in_y[i:i+timestep];
	print(x_train.shape);
	print(y_train.shape);
...
print("loading data");
x_data,y_data,x_paths=load_data(x_folder,y_folder,22,23);
print("data loaded");
print(x_data.shape);
print(y_data.shape);
timestep=20;
lrcn=getmodel(timestep);
x_train, y_train= format_data(x_data,y_data,timestep)
lrcn.fit(x_train,y_train,batch_size=1,epochs=1);
